(() => {
  ...
  style.textContent = `
    .asset-publisher { display:flex; align-items:center; gap:5px; margin:5px 0 0; color:var(--muted); font-size:.76rem; line-height:1.35; }
    .asset-publisher strong { color:#9bc9ff; font-weight:800; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:210px; }
    .asset-publisher-dot { width:5px; height:5px; flex:0 0 auto; border-radius:50%; background:#79b8ff; box-shadow:0 0 7px rgba(121,184,255,.45); }
  `;
  ...
  async function getClient() {
    ...
    clientPromise = (async () => {
      ...
      if (!window.supabase?.createClient || !cfg.url || !cfg.publishableKey) throw new Error("Supabase no está disponible para resolver publicadores.");
      return window.supabase.createClient(cfg.url, cfg.publishableKey, { auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false } });
    })();
  ...
  async function resolveAssets(client, ids) {
    ...
    for (const batch of chunks(missing)) {
      const { data, error } = await client.from("assets").select("id,submitted_by").in("id", batch);
      if (error) throw error;
      (Array.isArray(data) ? data : []).forEach((row) => assetSubmitters.set(String(row.id), row.submitted_by ? String(row.submitted_by) : null));
      batch.forEach((id) => { if (!assetSubmitters.has(id)) assetSubmitters.set(id, null); });
    }
  }
  ...
  async function resolveProfiles(client, userIds) {
    ...
    for (const batch of chunks(missing)) {
      const { data, error } = await client.from("toolhub_profiles").select("user_id,username,display_name").in("user_id", batch);
      if (error) throw error;
      ...
      batch.forEach((id) => { if (!profileNames.has(id)) profileNames.set(id, "Usuario ToolHub"); });
    }
  }
  ...
  async function decorate() {
    ...
    running = true;
    try {
      const cards = [...grid.querySelectorAll(".asset-card")].filter((card) => String(card.dataset.assetId || "").trim());
      if (!cards.length) return;
      const client = await getClient();
      const ids = cards.map((card) => String(card.dataset.assetId || "")).filter(Boolean);
      await resolveAssets(client, ids);
      await resolveProfiles(client, ids.map((id) => assetSubmitters.get(id)).filter(Boolean));
      cards.forEach((card) => {
        const id = String(card.dataset.assetId || "");
        const userId = assetSubmitters.get(id);
        addPublisher(card, userId ? profileNames.get(userId) || "Usuario ToolHub" : "Invitado");
      });
    } catch (error) {
  ...
  document.addEventListener("toolhub-assets-rendered", (event) => {
    ...
    assets.forEach((asset) => { if (asset?.id) assetSubmitters.set(String(asset.id), asset.submitted_by ? String(asset.submitted_by) : null); });
    schedule();
  });
