(() => {
  ...
  function renderAdminState() {
    ...
    els.adminStatus.textContent = state.adminUnlocked
      ? "🔓 Administrador · borrado habilitado"
      : "🔒 Administrador bloqueado · solo afecta al borrado";
    // Crear/publicar es público. El administrador solo controla las X de borrado.
  ...
  async function verifyAdmin() {
    ...
    try {
      ...
      setAdminMessage("Administrador conectado. Ya puedes borrar publicaciones con la X.", "ok");
      render();
  ...
  async function publishAsset(asset) {
    if (!state.apiBase) {
      alert("La Biblioteca todavía no tiene conectada la API compartida. Recarga la página e inténtalo de nuevo.");
      return false;
    }
  ...
  async function migrateLegacyAssets() {
    ...
    if (!state.apiBase) {
      setAdminMessage("La API compartida todavía no está disponible.", "error");
      return;
    }
  ...
  els.adminLock.addEventListener("click", () => {
    state.adminUnlocked = false;
    state.adminPassword = "";
    setAdminMessage("Modo administrador bloqueado. La publicación sigue siendo pública.");
    render();
  });
